...
# from log.textbox_handler import TextboxHandler
        ...
        try:
            ...
            if System == "Taimei":
                for i in DATASET_LIST:
                    try:
                        ...
                        locals()[str(i)] = taimei(PATH_INPUT_Old, PATH_INPUT_New, i,
                                                  PATH_INPUT_not_compare_variables).data
                        ...
                        raise e
                        # logging.warning("不参与的比较变量不在数据集{0}中，或系统变量不全，请确认源数据".format(i))
                ...
                for i in DATASET_LIST:
                    try:
                        logging.warning("开始比较数据集{0}".format(i))
                        locals()[str(i)] = cims(PATH_INPUT_Old, PATH_INPUT_New, i, PATH_INPUT_not_compare_variables).data
                        locals()[str(i)] = locals()[str(i)].drop(columns=PATH_INPUT_drop_variables)
                        locals()[str(i)] = locals()[str(i)].style.apply(highlight_rows, axis=1).applymap(
                            highlight_cells)
                    except Exception as e:
            ...
            # elif System == "Bioknow":
            #     ae = Bioknow(PATH_INPUT_Old, PATH_INPUT_New, "ae", PATH_INPUT_not_compare_variables).data
            # for i in DATASET_LIST:
            #     try:
            #         logging.warning("开始比较数据集{0}".format(i))
            #         locals()[str(i)] = Bioknow(PATH_INPUT_Old, PATH_INPUT_New, i,
            #                                      PATH_INPUT_not_compare_variables).data
            # locals()[str(i)] = locals()[str(i)].drop(columns=PATH_INPUT_drop_variables)
            # locals()[str(i)] = locals()[str(i)].style.apply(highlight_rows, axis=1).applymap(
            #     highlight_cells)
            ...
            with pandas.ExcelWriter("{0}\\{1}".format(PATH_OUTPUT, file),
                ...
                # test
                # ae.to_excel(writer,'AE', index=False)
                # 有目录文件就生成目录，没有就跳过
