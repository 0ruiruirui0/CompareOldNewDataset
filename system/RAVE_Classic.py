...
class Rave_C:
    ...
    def data(self):
        ...
        Compare_Variables = get_compare_variables_RAVE(data_New, filtered_not_compare_variables)
        data = compare_data_Rave(data_Old, data_New, Compare_Variables, filtered_not_compare_variables,
...
def get_compare_variables_RAVE(data: pd.DataFrame, not_compare_variables: list):
    system_1 = ['project', "SiteNumber", "Subject", "Folder", "FolderSeq", "InstanceName",
                "InstanceRepeatNumber", "DataPageName", "PageRepeatNumber","RecordPosition"]
...
def compare_data_Rave(OldData: pd.DataFrame, NewData: pd.DataFrame, Variables: list,
                      not_compare_variables: list, KEEP_DELETE_DATA: bool, COLOUR: bool, myconfig) -> pd.DataFrame:
    system_variables_1 = ['project', "SiteNumber", "Subject", "Folder", "FolderSeq",
...
def clean_sort_data_rave(data: pd.DataFrame, myconfig) -> pd.DataFrame:
    ...
    SORT_COLUMNS = ["sitenumber", "subject", "folderseq", "instancerepeatnumber", "datapagename",
                    "recordposition"]
    ...
    data_clean["instancerepeatnumber"] = data_clean["instancerepeatnumber"].astype(int, errors='ignore').astype(str,
                                                                                                                errors='ignore')
    data_clean["recordposition"] = data_clean["recordposition"].astype(int, errors='ignore').astype(str,
                                                                                                    errors='ignore')
    return data_clean
